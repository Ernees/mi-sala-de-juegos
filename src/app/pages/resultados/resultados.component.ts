...
export class ResultadosComponent {
  ...
  // constructor(){
  // this.db.traerResultados().then((response) => {
  //   if(response !== null) {
  //     this.resultados.set([...response]);
    
  //   }
  //     this.db.canal.on("postgres_changes", { event: "INSERT", schema: 'public', table: 'resultados',
  //     }, async (payload) => {
  //       const array: Resultados[] = this.resultados();
  //       array.push(payload.new as Resultados);
  //       this.resultados.set([...array]);
  //     });
  ...
  const rankingFinal = Array.from(rankingMap.values()).sort(
  ...
  // async ngOnInit(){
  //   const resultados= await this.db.traerResultados();
  //   const usuarios:any = [];
  //   console.log(resultados);
    // for (const r of this.resultados) {
    //   if (!usuarios.includes(r.id_usuario)) {
    //     usuarios.push(r.id_usuario);
    //   }
    // }
  }
